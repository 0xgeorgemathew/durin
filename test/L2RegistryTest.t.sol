...
contract L2RegistryTest is Test {
    ...
    function test_AddRegistrarUnauthedReverts() public {
        vm.expectRevert(abi.encodeWithSelector(IL2Resolver.Unauthorized.selector, uint256(registry.baseNode())));
        vm.prank(user1);
        registry.addRegistrar(address(registrar));
    }
    ...
    function testFuzz_RegisterTwiceReverts(
        string calldata label
    ) public {
        vm.assume(bytes(label).length > 1 && bytes(label).length < 255);
        ...
        vm.expectRevert(abi.encodeWithSelector(L2Registry.NotAvailable.selector, label, registry.baseNode()));
        registrar.register(label, admin);
        vm.stopPrank();
    }
    ...
    function testFuzz_CreateSubnodeWithData(
        ...
        data[0] = abi.encodeWithSelector(bytes4(0xd5fa2b00), expectedNodeFor4ld, user1);
        data[1] = abi.encodeWithSelector(IL2Resolver.setText.selector, expectedNodeFor4ld, "key", "value");

        bytes32 actualNodeFor4ld = registry.createSubnode(nodeFor3ld, sublabel, user1, data);
    ...
    function testFuzz_CreateSubnodeWithDataForUnownedNodeReverts(
        ...
        data[1] = abi.encodeWithSelector(IL2Resolver.setText.selector, nodeFor3ld, "key", "value");
    ...
    function testFuzz_CreateSubnodeFromUnapprovedNodeReverts(
        ...
        vm.expectRevert(abi.encodeWithSelector(IL2Resolver.Unauthorized.selector, uint256(nameNode)));
        vm.prank(user2);
        registry.createSubnode(nameNode, sublabel, user1, new bytes[](0));
    }

    function testFuzz_SetSingleRecordsByOwner(
        string calldata label
    ) public {
        vm.assume(bytes(label).length > 1 && bytes(label).length < 255);
    ...
    function testFuzz_SetTextRecordWithSignature(
        string calldata label
    ) public {
        vm.assume(bytes(label).length > 1 && bytes(label).length < 255);
        ...
        bytes32 message = keccak256(abi.encodePacked(address(registry), node, "key", "value", expiration))
        ...
        vm.prank(user2);
        registry.setTextWithSignature(node, "key", "value", expiration, user1, signature);
    ...
    function testFuzz_SetTextRecordWithSignatureByUnauthorizedAddressReverts(
        ...
        vm.prank(user2);
        vm.expectRevert(abi.encodeWithSelector(IL2Resolver.Unauthorized.selector, node));
        registry.setTextWithSignature(node, "key", "value", expiration, user2, signature);
    }

    function testFuzz_SetRecordUnauthedReverts(
        string calldata label
    ) public {
        vm.assume(bytes(label).length > 1 && bytes(label).length < 255);
        bytes32 node = registry.makeNode(registry.baseNode(), label);
    ...
    function testFuzz_SetRecordsWithMulticall(
        string calldata label
    ) public {
        vm.assume(bytes(label).length > 1 && bytes(label).length < 255);
        bytes32 node = registry.makeNode(registry.baseNode(), label);
        ...
        data[1] = abi.encodeWithSelector(IL2Resolver.setText.selector, node, "key", "value");
        data[2] = abi.encodeWithSelector(IL2Resolver.setContenthash.selector, node, hex"1234");
    ...
    function test_MulticallDoesntBlowEverythingUp() public {
        ...
        vm.startPrank(user1);
        data[0] = abi.encodeWithSelector(bytes4(0xd5fa2b00), registry.baseNode(), user1);
        vm.expectRevert();
    ...
    function test_RegistryHelperFunctions() public {
        ...
        assertEq(node, 0x9709c900112b2537a4268551c6a89092af6ce2e45a001af4e8dc5d800c4eae25);
    ...
    function test_TokenMetadata() public {
        ...
        string memory metadata =
            string.concat("data:application/json;base64,", Base64.encode(bytes('{"name": "testname.eth"}')));
        ...
        assertEq(registry.tokenURI(uint256(testNameNode)), string(abi.encodePacked(baseURI, tokenIdString)));
    }
}
